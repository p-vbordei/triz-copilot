...
class TRIZKnowledgeIngestion:
    ...
    def ingest_triz_principles(
        ...
        for principle_id, principle in knowledge_base.principles.items():
            ...
            if embedding is not None:
                ...
                logger.info(
                    f"  Processed principle {principle_id}: {principle.principle_name}"
                )
    ...
    def ingest_contradiction_matrix(
        ...
        if matrix_file is None:
            matrix_file = (
                Path(__file__).parent.parent / "data" / "contradiction_matrix.json"
            )
        ...
        try:
            ...
            if vectors:
                ...
                if success:
                    self.ingested_count += len(vectors)
                    logger.info(
                        f"✅ Ingested {len(vectors)} chunks from {pdf_path.name}"
                    )
                    return True
        ...
        for file_path in files:
            ...
            if file_path.suffix.lower() == ".pdf":
                success = self.ingest_pdf_content(
                    file_path, collection_name=collection_name
                )
...
def main():
    ...
    parser.add_argument(
        "--principles", action="store_true", help="Ingest TRIZ principles"
    )
    parser.add_argument(
        "--matrix", action="store_true", help="Ingest contradiction matrix"
    )
    ...
    if args.directory:
        ...
        pipeline.ingest_directory(dir_path, max_files=args.max_files)
